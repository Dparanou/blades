...
import {DetailsSubscribersComponent} from "../bladers/details-subscribers/details-subscribers.component";
                    ...
                    if (foundChild && foundChild._loadedConfig.routes.length > 0) {
                        ...
                        foundChild._loadedConfig.routes.forEach(childRoute => {
                            if (childRoute.data && childRoute.data.addDynamicChild) {
                                ...
                                const foundDynamicChild = childRoute.children.find(child => child.path === 'details');
                                if (!foundDynamicChild) {

                                    childRoute.children.push(
                                        {
                                            path: 'details',
                                            component: DetailsSubscribersComponent
                                        }
                                    );
